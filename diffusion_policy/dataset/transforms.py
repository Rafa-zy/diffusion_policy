...
def _build_transform_train(cfg, choices, target_size, normalize):
    ...
    input_size = (96,96)  #cfg.INPUT.SIZE
...
def build_transform_val(cfg, is_train=True, choices=None):
    if cfg.INPUT.NO_TRANSFORM:
        print("Note: no transform is applied!")
        return None
    ...
    return _build_transform_val(cfg, choices, target_size, normalize)

def _build_transform_val(cfg, choices, target_size, normalize):
    ...
    tfm_test += [Resize((max(input_size), max(input_size)), interpolation=interp_mode)]
...
def _build_transform_test(cfg, choices, target_size, normalize):
    ...
    for choice in choices: 
        ...
        if "random_flip" == choice:
            print("+ random flip")
            tfm_test += [RandomHorizontalFlip()]

        if "random_resized_crop" == choice:
            s_ = cfg.INPUT.RRCROP_SCALE
            print(f"+ random resized crop (size={input_size}, scale={s_})")
            tfm_test += [
                RandomResizedCrop(input_size, scale=s_, interpolation=interp_mode)]
        ...
        if "gaussian_blur" == choice:
            print(f"+ gaussian blur (kernel={cfg.INPUT.GB_K})")
            gb_k, gb_p = cfg.INPUT.GB_K, cfg.INPUT.GB_P
            tfm_test += [RandomApply([GaussianBlur(gb_k)], p=gb_p)]
